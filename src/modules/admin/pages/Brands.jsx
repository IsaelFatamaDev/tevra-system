    ...
    <div className="max-w-5xl mx-auto space-y-5 platform-enter">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-[#134074] mt-0.5">{t('admin.brands.subtitle')}</p>
        </div>
      ...
      <div className="flex gap-3">
        <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
          ...
          <div>
            <p className="text-xs text-[#134074]">{t('admin.brands.totalBrands')}</p>
            <p className="text-lg font-semibold text-[#134074]">{brands.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-[#C5D8E8]/20 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <span className="material-symbols-outlined text-3xl text-[#13315C]">storefront</span>
            <p className="text-sm text-[#134074] mt-2">{t('admin.brands.noBrandsFound')}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[#C5D8E8]/10">
                {paginated.map(brand => (
                  <tr key={brand.id} className="hover:bg-[#EEF4ED]/30 transition-colors group">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        {brand.logoUrl ? (
                          <img src={brand.logoUrl} alt={brand.name} className="w-9 h-9 rounded-lg object-contain bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 p-0.5" />
                        ) : (
                          <div className="w-9 h-9 rounded-lg bg-[#EEF4ED] flex items-center justify-center text-[#134074]">
                            <span className="material-symbols-outlined text-[18px]">storefront</span>
                          </div>
                        )}
                        <span className="text-sm font-medium text-[#134074]">{brand.name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-sm text-[#134074]">{new Date(brand.createdAt).toLocaleDateString('es-PE')}</td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        <button onClick={() => openDelete(brand)} title={t('common.delete')}
                          className="p-1.5 rounded-md hover:bg-red-50 text-[#13315C] hover:text-red-500 transition-colors opacity-60 group-hover:opacity-100">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-[#C5D8E8]/10 flex justify-between items-center bg-[#EEF4ED]/30/50">
              <h3 className="text-lg font-semibold text-[#134074]">{modal === 'create' ? t('admin.brands.newBrand') : t('admin.brands.editBrand')}</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <button type="button" onClick={() => fileRef.current?.click()}
                  className="w-full flex items-center gap-4 p-4 bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-xl hover:border-tevra-coral hover:bg-white transition-all group shadow-sm focus:outline-none focus:ring-2 focus:ring-tevra-coral/20">
                  {logoPreview ? (
                    <img src={logoPreview} alt="Preview" className="w-14 h-14 rounded-lg object-contain bg-white border border-[#C5D8E8]/20 p-1 shadow-sm group-hover:scale-105 transition-transform" />
                  ) : (
                    <div className="w-14 h-14 rounded-lg bg-white border border-[#C5D8E8]/20 flex items-center justify-center text-[#13315C] group-hover:text-tevra-coral shadow-sm group-hover:border-tevra-coral transition-colors">
                      <span className="material-symbols-outlined text-2xl">add_photo_alternate</span>
                    </div>
                  )}
                  <div className="text-left flex-1">
                    <p className="text-sm font-bold text-[#134074] group-hover:text-tevra-coral transition-colors decoration-2 underline-offset-2">{t('admin.brands.browseFiles')}</p>
                    <p className="text-xs text-[#134074] font-medium mt-0.5">{t('admin.brands.fileHint')}</p>
                  </div>
                </button>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.brands.institutionalName')}</label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            <div className="p-6 text-center">
              ...
              <h3 className="text-xl font-bold text-[#134074] mb-2">{t('admin.brands.deleteTitle')}</h3>
              <p className="text-sm text-[#134074] mb-6 leading-relaxed">
                {t('admin.brands.deleteConfirmation', { name: selected?.name })}
              </p>

              <div className="flex gap-3 w-full">
                <button onClick={() => setModal(null)} className="flex-1 py-2.5 bg-white border border-[#C5D8E8]/20 text-sm font-semibold text-[#134074] hover:bg-[#EEF4ED]/30 hover:text-[#134074] rounded-xl transition-colors shadow-sm">
                  {t('common.cancel')}
                </button>
                <button onClick={handleDelete} disabled={saving}
                  className="flex-1 py-2.5 bg-red-500 hover:bg-red-600 disabled:opacity-50 text-[#EEF4ED] rounded-xl font-semibold text-sm transition-all shadow-md hover:-translate-y-0.5 disabled:translate-y-0 disabled:shadow-none flex items-center justify-center gap-2">
                  {saving && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
