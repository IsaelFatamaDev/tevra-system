...
export default function AdminCommunications() {
  ...
  const channelColors = { email: 'bg-[#EEF4ED] text-[#134074]', whatsapp: 'bg-[#EEF4ED] text-[#134074]', sms: 'bg-[#EEF4ED] text-[#134074]', push: 'bg-[#EEF4ED] text-[#134074]' }
  const STATUS_CONFIG = {
    draft: { bg: 'bg-[#EEF4ED]', text: 'text-[#134074]', label: t('admin.communications.statusDraft') },
    ...
    <div className="max-w-7xl mx-auto space-y-5 platform-enter">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          ...
          <p className="text-sm text-[#134074] mt-0.5">{t('admin.communications.subtitle')}</p>
        </div>
        <button onClick={() => setShowCreate(true)}
          className="bg-[#134074] hover:bg-primary-mid text-[#EEF4ED] px-4 py-2 rounded-lg font-medium flex items-center justify-center gap-1.5 transition-colors text-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          <div key={i} className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
            <div className="w-9 h-9 rounded-lg bg-[#EEF4ED] text-[#134074] flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-[#C5D8E8]/20">
              <span className="material-symbols-outlined text-3xl text-[#13315C]">campaign</span>
              <p className="text-sm text-[#134074] mt-2">{t('admin.communications.noCampaigns')}</p>
            </div>
          ) : (
            <div className="space-y-2">
              {paginatedCampaigns.map((camp, i) => {
                ...
                return (
                  <div key={camp.id || i} className="group bg-white p-3.5 rounded-xl border border-[#C5D8E8]/20 transition-all hover:border-[#C5D8E8]/40 flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-lg flex items-center justify-center shrink-0 ${channelColors[camp.type] || 'bg-[#EEF4ED] text-[#134074]'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-[10px] text-[#13315C]">{new Date(camp.sentAt || camp.scheduledAt || camp.createdAt).toLocaleDateString('es-PE')}</span>
                      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button key={i} onClick={() => applyTemplate(tpl.key)} className="bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 p-3 rounded-lg text-center hover:bg-[#EEF4ED] transition-colors">
                  <span className="material-symbols-outlined text-xl mb-1 text-[#134074]">{tpl.icon}</span>
                  <p className="text-[10px] font-medium text-[#134074] uppercase">{tpl.label}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#134074] text-[#EEF4ED] p-4 rounded-xl overflow-hidden relative">
            <div className="relative z-10">
              ...
              <p className="text-[#EEF4ED]/60 text-xs mb-3">{t('admin.communications.segmentationHint')}</p>
              <div className="space-y-2">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between text-xs font-medium py-2 border-b border-white/10 last:border-transparent">
                    <span>{seg.label}</span>
                    <span className="bg-white/10 text-[#EEF4ED] px-2 py-0.5 rounded text-[10px] font-medium">{seg.count}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-[#134074] text-sm">{t('admin.communications.deliverability')}</h3>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex-1 h-1.5 bg-[#EEF4ED] rounded-full overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full" style={{ width: '98%' }}></div>
              </div>
              <span className="font-semibold text-[#134074] text-sm">98%</span>
            </div>
            <p className="text-[10px] text-[#134074] mt-2">{t('admin.communications.domainReputation')}</p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-4 border-b border-[#C5D8E8]/10 flex justify-between items-center">
              <h3 className="text-base font-semibold text-[#134074]">{t('admin.communications.newCampaign')}</h3>
              <button onClick={() => setShowCreate(false)} className="p-1 hover:bg-[#EEF4ED] rounded-md"><span className="material-symbols-outlined text-[#13315C] text-[18px]">close</span></button>
            </div>
            <div className="p-4 space-y-3">
              <div>
                <label className="block text-xs font-medium text-[#134074] mb-1">{t('admin.communications.campaignName')}</label>
                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder={t('admin.communications.campaignPlaceholder')}
                  className="w-full px-3 py-2 bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-lg text-sm focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-[#134074] mb-1">{t('admin.communications.channel')}</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-[#134074] mb-1">{t('admin.communications.audience')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#134074] mb-1">{t('admin.communications.estimatedRecipients')}</label>
                <input type="number" value={form.recipientCount} onChange={e => setForm({ ...form, recipientCount: Number(e.target.value) })}
                  className="w-full px-3 py-2 bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-lg text-sm focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#134074] mb-1">{t('admin.communications.message')}</label>
                <textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} rows={3} placeholder={t('admin.communications.messagePlaceholder')}
                  className="w-full px-3 py-2 bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-lg text-sm focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none resize-none" />
              </div>
            </div>
            <div className="p-4 border-t border-[#C5D8E8]/10 flex justify-end gap-2">
              <button onClick={() => setShowCreate(false)} className="px-4 py-2 text-sm font-medium text-[#134074] hover:bg-[#EEF4ED]/30 rounded-lg transition-colors">{t('common.cancel')}</button>
              <button onClick={handleCreate} disabled={saving || !form.name}
                className="px-4 py-2 text-sm font-medium bg-[#134074] text-[#EEF4ED] rounded-lg hover:bg-primary-mid disabled:opacity-40 transition-colors">
