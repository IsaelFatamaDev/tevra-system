...
export default function AdminAgents() {
  ...
  const STATUS_CONFIG = {
    active: { bg: 'bg-status-success/10', text: 'text-status-success', dot: 'bg-status-success', label: t('admin.agents.statusActive') },
    pending: { bg: 'bg-status-warning/10', text: 'text-status-warning', dot: 'bg-status-warning', label: t('admin.agents.statusPending') },
    inactive: { bg: 'bg-status-pending/20', text: 'text-primary', dot: 'bg-status-pending', label: t('admin.agents.statusInactive') },
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 platform-enter">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          <h2 className="text-xl font-semibold text-[#134074]">{t('admin.agents.title')}</h2>
          <p className="text-sm text-[#134074] mt-0.5">{t('admin.agents.subtitle')}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-[#C5D8E8]/20 overflow-hidden">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
                <button key={key} onClick={() => setStatusFilter(statusFilter === key ? '' : key)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === key ? 'bg-[#134074] text-[#EEF4ED]' : 'bg-[#EEF4ED]/30 text-[#134074] hover:bg-[#EEF4ED] border border-[#C5D8E8]/20'}`}>
                  {cfg.label}
                </button>
              ))}
              {cities.length > 0 && (
                <select value={cityFilter} onChange={e => setCityFilter(e.target.value)}
                  className="px-3 py-1.5 bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-lg text-xs font-medium text-[#134074] focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none">
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              <span className="material-symbols-outlined text-3xl text-[#13315C]">person_off</span>
              <p className="text-sm text-[#134074] mt-2">{t('admin.agents.noAgentsFound')}</p>
            </div>
          ) : (
              ...
              <table className="w-full text-left min-w-150">
                ...
                <tbody className="divide-y divide-[#C5D8E8]/10">
                  {paginated.map(a => {
                    ...
                    return (
                      <tr key={a.id} className={`hover:bg-[#EEF4ED]/30 transition-colors group ${a.status === 'inactive' ? 'opacity-60' : ''}`}>
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-3">
                            {a.avatarUrl ? (
                              <img src={a.avatarUrl} alt={fullName} className="w-9 h-9 rounded-full object-cover" />
                            ) : (
                              <div className="w-9 h-9 rounded-full bg-[#EEF4ED] flex items-center justify-center text-[#134074] font-medium text-sm">
                                {(a.firstName?.[0] || '') + (a.lastName?.[0] || '')}
                              </div>
                            )}
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-[#134074]">{fullName}</p>
                              <p className="text-xs text-[#134074] truncate">{a.email}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-sm text-[#134074]">{a.city || '—'}</td>
                        <td className="px-5 py-3">
                          <div className="flex flex-col">
                            <span className="text-sm font-medium text-[#134074]">{a.totalSales || 0} {t('admin.agents.salesCount')}</span>
                            {a.rating ? (
                              <div className="flex items-center gap-1">
                                <span className="material-symbols-outlined text-amber-400 text-[14px]" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                                <span className="text-xs text-[#134074]">{Number(a.rating).toFixed(1)}</span>
                              </div>
                            ) : <span className="text-xs text-[#13315C]">{t('admin.agents.noRating')}</span>}
                          </div>
                        </td>
                        <td className="px-5 py-3">
                        ...
                        <td className="px-5 py-3 text-right">
                          <div className="flex justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => { setStatusModal(a); setNewStatus(a.status) }} title={t('admin.agents.changeStatus')}
                              className="p-1.5 rounded-md hover:bg-[#EEF4ED] text-[#13315C] hover:text-amber-600 transition-colors">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20">
            <div className="flex justify-between items-center mb-3">
              ...
              {pendingCount > 0 && (
                <span className="bg-[#134074] text-[#EEF4ED] text-[10px] font-medium px-2 py-0.5 rounded-full">{pendingCount}</span>
              )}
            </div>
            <div className="space-y-2">
              {pendingApps.map(req => (
                <div key={req.id} className="flex gap-3 p-2.5 rounded-lg bg-[#EEF4ED]/30 hover:bg-[#EEF4ED] transition-colors">
                  <div className="w-8 h-8 rounded-full bg-[#C5D8E8]/20 flex items-center justify-center text-[#134074] font-medium text-xs shrink-0">
                    {(req.fullName?.split(' ')[0]?.[0] || '') + (req.fullName?.split(' ')[1]?.[0] || '')}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#134074] truncate">{req.fullName}</p>
                    <p className="text-[11px] text-[#134074]">{req.city}</p>
                  </div>
                  <button onClick={() => { setReviewApp(req); setRejectStep(false); setRejectNotes('') }}
                    className="px-2.5 py-1 text-[11px] font-semibold bg-[#134074] text-[#F4E9CD] rounded-lg hover:bg-[#8DA9C4] transition-colors whitespace-nowrap shrink-0">
                    Ver
                  </button>
                </div>
              ))}
              {pendingCount === 0 && <p className="text-sm text-[#134074] text-center py-3">{t('admin.agents.noPendingApplications')}</p>}
            </div>
          </div>
      ...
      {reviewApp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col relative z-10">

            {/* Header */}
            <div className="px-6 py-4 border-b border-[#C5D8E8]/10 flex justify-between items-center rounded-t-2xl bg-[#EEF4ED]/30">
              <div>
                <h3 className="text-base font-semibold text-[#134074]">
                  {rejectStep ? 'Motivo de rechazo' : 'Solicitud de agente'}
                </h3>
                <p className="text-xs text-[#134074] mt-0.5">{reviewApp.fullName}</p>
              </div>
              <button onClick={() => { setReviewApp(null); setRejectStep(false); setRejectNotes('') }}
                className="p-2 hover:bg-[#EEF4ED] rounded-full transition-colors text-[#13315C]">
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              {!rejectStep ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">Nombre completo</p>
                      <p className="text-sm font-semibold text-[#134074]">{reviewApp.fullName}</p>
                    </div>
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">DNI / Documento</p>
                      <p className="text-sm font-semibold text-[#134074]">{reviewApp.dni || '—'}</p>
                    </div>
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">Correo</p>
                      <p className="text-sm text-[#134074] break-all">{reviewApp.email}</p>
                    </div>
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">WhatsApp</p>
                      <p className="text-sm text-[#134074]">{reviewApp.whatsapp || '—'}</p>
                    </div>
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">Ciudad</p>
                      <p className="text-sm font-semibold text-[#134074]">{reviewApp.city || '—'}</p>
                    </div>
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      <p className="text-[10px] font-bold uppercase tracking-wider text-[#13315C] mb-0.5">Fecha de solicitud</p>
                  ...
                  {reviewApp.motivation && (
                    <div className="p-3 bg-[#EEF4ED]/40 rounded-xl border border-[#C5D8E8]/10">
                      ...
                      <p className="text-sm text-[#134074] leading-relaxed">{reviewApp.motivation}</p>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {reviewApp.categories.map((cat, i) => (
                          <span key={i} className="px-2.5 py-1 bg-[#134074] text-[#F4E9CD] text-xs font-medium rounded-lg">{cat}</span>
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {reviewApp.coverageAreas.map((area, i) => (
                          <span key={i} className="px-2.5 py-1 bg-[#EEF4ED] text-[#134074] text-xs font-medium rounded-lg border border-[#C5D8E8]/20">{area}</span>
                        ))}
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-[#134074] mb-1.5">
                    ...
                    <textarea
                      ...
                      className="w-full px-3 py-2.5 bg-[#EEF4ED]/30 border border-[#C5D8E8]/30 rounded-xl text-sm text-[#134074] placeholder-[#C5D8E8] focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none resize-none transition-all"
                    />
                    <p className="text-[11px] text-[#13315C] mt-1">{rejectNotes.length} caracteres</p>
                  </div>
                </div>
              )}
            </div>
              ...
              {!rejectStep ? (
                <>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { setReviewApp(null); setRejectStep(false) }}
                      className="px-4 py-2 text-sm text-[#134074] bg-white border border-[#C5D8E8]/20 rounded-xl hover:bg-[#EEF4ED]/30 transition-colors">
                      Cerrar
                    </button>
                ...
                <>
                  <button onClick={() => setRejectStep(false)}
                    className="px-4 py-2 text-sm text-[#134074] bg-white border border-[#C5D8E8]/20 rounded-xl hover:bg-[#EEF4ED]/30 transition-colors flex items-center gap-1.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl lg:max-w-2xl max-h-[85vh] flex flex-col relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-[#C5D8E8]/10 flex justify-between items-center bg-[#EEF4ED]/30/50 rounded-t-2xl">
              <h3 className="text-lg font-semibold text-[#134074]">{t('admin.agents.agentProfile')}</h3>
              <button onClick={() => setViewAgent(null)} className="p-2 hover:bg-[#EEF4ED] rounded-full transition-colors -mr-2 text-[#13315C]">
                <span className="material-symbols-outlined text-[20px]">close</span>
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="flex items-center gap-4">
                {viewAgent.avatarUrl ? (
                  <img src={viewAgent.avatarUrl} alt="" className="w-16 h-16 rounded-full object-cover shadow-sm ring-1 ring-[#C5D8E8]/30" />
                ) : (
                ...
                <div>
                  <h4 className="text-lg font-bold text-[#134074] leading-tight">{viewAgent.firstName} {viewAgent.lastName}</h4>
                  <p className="text-sm font-medium text-[#134074] mb-1">{viewAgent.email}</p>
                  {/* Status badge */}
                  {(() => {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.totalSales')}</p><p className="font-bold text-[#134074] text-lg">{viewAgent.totalSales || 0}</p></div>
                <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.revenueRev')}</p><p className="font-bold text-emerald-600 text-lg">${Number(viewAgent.totalRevenue || 0).toLocaleString()}</p></div>
                <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.city')}</p><p className="font-semibold text-[#134074]">{viewAgent.city || '—'}</p></div>
                <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.agreedCommission')}</p><p className="font-semibold text-[#134074]">{viewAgent.commissionRate || 0}%</p></div>
                {viewAgent.referralCode && <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.referralCode')}</p><p className="font-mono font-bold text-sm text-[#134074]">{viewAgent.referralCode}</p></div>}
                {viewAgent.createdAt && <div className="p-3 bg-[#EEF4ED]/30 rounded-xl border border-[#C5D8E8]/10"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-0.5">{t('admin.agents.memberSince')}</p><p className="font-semibold text-sm text-[#134074]">{new Date(viewAgent.createdAt).toLocaleDateString('es-PE')}</p></div>}
              </div>

              {viewAgent.zones?.length > 0 && (
                <div className="p-4 bg-[#EEF4ED]/30 border border-[#C5D8E8]/10 rounded-xl">
                  <p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-2">{t('admin.agents.coverageZones')}</p>
                  <div className="flex flex-wrap gap-1.5">{viewAgent.zones.map((z, i) => <span key={i} className="px-2.5 py-1 bg-white text-[#134074] text-xs font-semibold rounded-lg shadow-sm border border-[#C5D8E8]/20">{z}</span>)}</div>
                </div>
              )}
              {viewAgent.bio && <div className="p-4 bg-[#EEF4ED]/30 border border-[#C5D8E8]/10 rounded-xl"><p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-1.5">{t('admin.agents.bio')}</p><p className="text-sm text-[#134074] leading-relaxed">{viewAgent.bio}</p></div>}
              {viewAgent.specializationCategories?.length > 0 && (
                <div>
                  <p className="text-[11px] uppercase tracking-wider font-bold text-[#134074] mb-2">{t('admin.agents.specialties')}</p>
                  <div className="flex flex-wrap gap-2">
                    {viewAgent.specializationCategories.map((cat, i) => (
                      <span key={i} className="px-3 py-1 bg-[#134074] text-[#EEF4ED] text-xs font-medium rounded-lg shadow-sm">{cat}</span>
                    ))}
                  </div>
                </div>
              )}
            ...
            <div className="p-5 border-t border-[#C5D8E8]/10 flex flex-col sm:flex-row justify-between items-center gap-4 bg-white rounded-b-2xl">
              <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-center sm:justify-start">
                ...
                <button onClick={() => { setViewAgent(null); setStatusModal(viewAgent); setNewStatus(viewAgent.status) }}
                  className="px-4 py-2 bg-[#EEF4ED] text-[#134074] font-bold text-sm rounded-xl hover:bg-[#C5D8E8]/20 transition-colors flex items-center gap-1.5 shadow-sm flex-1 sm:flex-none justify-center">
                  <span className="material-symbols-outlined text-[16px]">shield_person</span> {t('admin.agents.changeStatus')}
                </button>
              </div>
              <button onClick={() => setViewAgent(null)} className="w-full sm:w-auto px-6 py-2 font-semibold text-[#134074] bg-white hover:bg-[#EEF4ED]/30 border border-[#C5D8E8]/20 rounded-xl transition-colors shadow-sm text-sm">{t('admin.agents.closeProfile')}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {statusModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-[#C5D8E8]/10 flex justify-between items-start bg-[#EEF4ED]/30/50 rounded-t-2xl">
              <div>
                <h3 className="text-lg font-semibold text-[#134074]">{t('admin.agents.updateStatus')}</h3>
                <p className="text-sm font-medium text-[#134074] mt-0.5">{statusModal.firstName} {statusModal.lastName}</p>
              </div>
              <button onClick={() => setStatusModal(null)} className="p-2 hover:bg-[#EEF4ED] rounded-full transition-colors -mr-3 text-[#13315C]">
                <span className="material-symbols-outlined text-[20px]">close</span>
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 gap-2.5">
                {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
                  <button key={key} onClick={() => setNewStatus(key)}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all border ${newStatus === key ? 'bg-[#134074] text-[#EEF4ED] border-[#134074] shadow-md ring-1 ring-[#134074] ring-offset-1' : 'bg-white border-[#C5D8E8]/20 text-[#134074] hover:border-[#C5D8E8]/40 hover:bg-[#EEF4ED]/30'}`}>
            ...
            <div className="px-6 py-4 border-t border-[#C5D8E8]/10 flex justify-end gap-3 bg-[#EEF4ED]/30/50 rounded-b-2xl">
              <button onClick={() => setStatusModal(null)} className="px-4 py-2 bg-white border border-[#C5D8E8]/20 text-sm font-medium text-[#134074] hover:bg-[#EEF4ED]/30 hover:text-[#134074] rounded-xl transition-colors shadow-sm">{t('common.cancel')}</button>
              <button onClick={handleChangeStatus} disabled={saving || newStatus === statusModal.status}
                className="px-6 py-2 text-sm font-semibold bg-tevra-coral text-[#EEF4ED] rounded-xl shadow-md hover:-translate-y-0.5 hover:shadow-lg transition-all disabled:opacity-40 disabled:translate-y-0 disabled:shadow-none flex items-center gap-2">
