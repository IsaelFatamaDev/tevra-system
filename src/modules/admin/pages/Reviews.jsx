...
export default function AdminReviews() {
  ...
  const Stars = ({ rating, size = 16 }) => (
    <div className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={`material-symbols-outlined text-[${size}px] ${i < rating ? 'text-amber-400' : 'text-[#13315C]'}`}
    ...
    <div className="max-w-7xl mx-auto space-y-5 platform-enter">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
          ...
          <div>
            <p className="text-xs text-[#134074]">{t('admin.reviews.avgRating')}</p>
            <p className="text-lg font-semibold text-[#134074]">{avgRating.toFixed(1)}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
          <div className="w-9 h-9 rounded-lg bg-[#EEF4ED] text-[#134074] flex items-center justify-center shrink-0">
          ...
          <div>
            <p className="text-xs text-[#134074]">{t('admin.reviews.totalReviews')}</p>
            <p className="text-lg font-semibold text-[#134074]">{reviews.length}</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
          <div className="w-9 h-9 rounded-lg bg-[#EEF4ED] text-[#134074] flex items-center justify-center shrink-0">
          ...
          <div>
            <p className="text-xs text-[#134074]">{t('admin.reviews.verified')}</p>
            <p className="text-lg font-semibold text-[#134074]">{approvedCount} <span className="text-sm font-normal text-amber-600">({pendingCount} {t('admin.reviews.pendingBadge')})</span></p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20">
          <p className="text-xs text-[#134074] mb-2">{t('admin.reviews.distribution')}</p>
          <div className="space-y-1">
            {ratingDist.map(d => (
              <div key={d.star} className="flex items-center gap-1.5">
                <span className="text-[10px] text-[#134074] w-3 font-medium">{d.star}</span>
                ...
                <span className="text-[10px] text-[#134074] w-4 text-right">{d.count}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-[#C5D8E8]/20 overflow-hidden">
          ...
          <div className="flex gap-1.5 flex-wrap">
            {[5, 4, 3, 2, 1].map(star => (
              <button key={star} onClick={() => setRatingFilter(ratingFilter === star ? 0 : star)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all flex items-center gap-1 ${ratingFilter === star ? 'bg-[#134074] text-[#EEF4ED]' : 'bg-[#EEF4ED]/30 text-[#134074] hover:bg-[#EEF4ED] border border-[#C5D8E8]/20'}`}>
            ...
            {[{ key: '', label: t('common.all') }, { key: 'pending', label: t('admin.reviews.pendingBadge') }, { key: 'approved', label: t('admin.reviews.verifiedBadge') }, { key: 'rejected', label: t('common.rejected') }].map(opt => (
              <button key={opt.key} onClick={() => setStatusFilter(statusFilter === opt.key ? '' : opt.key)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${statusFilter === opt.key ? 'bg-[#134074] text-[#EEF4ED]' : 'bg-[#EEF4ED]/30 text-[#134074] hover:bg-[#EEF4ED] border border-[#C5D8E8]/20'}`}>
                {opt.label}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <span className="material-symbols-outlined text-3xl text-[#13315C]">rate_review</span>
            <p className="text-sm text-[#134074] mt-2">{t('admin.reviews.noReviewsFound')}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left min-w-162.5">
              ...
              <tbody className="divide-y divide-[#C5D8E8]/10">
                {paginated.map(rev => {
                  ...
                  return (
                    <tr key={rev.id} className="hover:bg-[#EEF4ED]/30 transition-colors group">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          {rev.reviewer?.avatarUrl ? (
                            ...
                            <div className="w-8 h-8 rounded-full bg-[#EEF4ED] flex items-center justify-center text-[#134074] font-medium text-xs">
                              {(rev.reviewer?.firstName?.[0] || 'A') + (rev.reviewer?.lastName?.[0] || '')}
                            </div>
                          )}
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-[#134074]">{reviewerName}</p>
                            <p className="text-xs text-[#134074] truncate">{rev.title || '—'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3"><Stars rating={rev.rating} /></td>
                      <td className="px-5 py-3 text-sm text-[#134074] max-w-50 truncate">{rev.comment || rev.body || '—'}</td>
                      <td className="px-5 py-3 text-sm text-[#134074]">{rev.createdAt ? new Date(rev.createdAt).toLocaleDateString('es-PE') : '—'}</td>
                      <td className="px-5 py-3">
                        {rev.status === 'approved' ? (
                          ...
                          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-[#EEF4ED] text-[#134074] text-[11px] font-medium rounded-md">
      ...
      {viewReview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={() => setViewReview(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            {/* Gradient header */}
            <div className="relative bg-linear-to-br from-[#134074] via-primary-mid to-[#134074] px-6 pt-6 pb-8">
              <button
                onClick={() => setViewReview(null)}
                className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors"
              >
                <span className="material-symbols-outlined text-[#EEF4ED] text-[16px]">close</span>
              </button>

              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-white/15 border border-white/20 flex items-center justify-center text-[#EEF4ED] font-bold text-lg shrink-0">
                  {(viewReview.reviewer?.firstName?.[0] || 'A')}{(viewReview.reviewer?.lastName?.[0] || '')}
                </div>
                <div>
                  <p className="text-[#EEF4ED] font-semibold text-base leading-tight">
                    {viewReview.reviewer ? `${viewReview.reviewer.firstName} ${viewReview.reviewer.lastName}` : t('admin.reviews.anonymous')}
                  </p>
                  <p className="text-[#EEF4ED]/50 text-xs mt-0.5">
              ...
              <div className="flex items-center gap-3 mt-5">
                <div className="flex gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <span key={i} className={`material-symbols-outlined text-2xl ${i < viewReview.rating ? 'text-amber-400' : 'text-[#EEF4ED]/15'}`}
                      style={{ fontVariationSettings: i < viewReview.rating ? "'FILL' 1" : "'FILL' 0" }}>star</span>
                  ))}
                </div>
                <span className="text-[#EEF4ED]/70 text-sm font-medium">{viewReview.rating}/5</span>
              </div>
            </div>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <blockquote className="relative pl-4 border-l-2 border-[#C5D8E8]/20">
                <p className="text-[#134074] text-sm leading-relaxed italic">"{viewReview.comment || viewReview.body || '—'}"</p>
              </blockquote>
              {viewReview.helpfulCount > 0 && (
                <p className="text-xs text-[#13315C] flex items-center gap-1.5">
              ...
              <div className="flex gap-2">
                {viewReview.status !== 'approved' && (
                  <button onClick={() => handleModerateReview(viewReview.id, 'approve')}
                    className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-[#EEF4ED] bg-emerald-600 hover:bg-emerald-700 rounded-xl transition-colors shadow-sm">
                ...
                {viewReview.status !== 'rejected' && (
                  <button onClick={() => handleModerateReview(viewReview.id, 'reject')}
                    className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-[#EEF4ED] bg-primary-mid hover:bg-[#8DA9C4] rounded-xl transition-colors shadow-sm">
                    <span className="material-symbols-outlined text-[16px]">block</span>
                    {t('common.reject')}
                  </button>
                )}
                <button onClick={() => setViewReview(null)}
                  className="px-4 py-2 text-xs font-semibold text-[#134074] hover:bg-[#EEF4ED] rounded-xl transition-colors">
                  {t('common.close')}
                </button>
              </div>
            </div>
