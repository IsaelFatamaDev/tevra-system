...
export default function AdminUsers() {
  ...
  const ROLE_CONFIG = {
    ...
    agent: { bg: 'bg-[#EEF4ED]', text: 'text-[#134074]', label: t('common.roles.agent') },
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 platform-enter">
      <div>
        <h2 className="text-xl font-semibold text-[#134074]">{t('admin.users.title')}</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {metrics.map((m, i) => (
          <div key={i} className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 flex items-center gap-3 stat-card">
            <div className="w-9 h-9 rounded-lg bg-[#EEF4ED] text-[#134074] flex items-center justify-center flex-shrink-0">
              <span className="material-symbols-outlined text-[18px]">{m.icon}</span>
            </div>
            <div>
              <p className="text-xs text-[#134074]">{m.title}</p>
      ...
      <div className="bg-white rounded-xl border border-[#C5D8E8]/20 overflow-hidden">
        <div className="p-4 border-b border-[#C5D8E8]/10 flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-1.5 flex-wrap">
            <button onClick={() => setRoleFilter('')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${!roleFilter ? 'bg-[#134074] text-[#EEF4ED]' : 'bg-[#EEF4ED]/30 text-[#134074] hover:bg-[#EEF4ED] border border-[#C5D8E8]/20'}`}>
              {t('common.all')}
            </button>
            {Object.entries(ROLE_CONFIG).map(([key, cfg]) => (
              <button key={key} onClick={() => setRoleFilter(roleFilter === key ? '' : key)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${roleFilter === key ? 'bg-[#134074] text-[#EEF4ED]' : 'bg-[#EEF4ED]/30 text-[#134074] hover:bg-[#EEF4ED] border border-[#C5D8E8]/20'}`}>
                {cfg.label}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-[#134074] mt-2">{t('admin.users.noUsers')}</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left min-w-[650px]">
              ...
              <tbody className="divide-y divide-[#C5D8E8]/10">
                {paginated.map(user => {
                  ...
                  return (
                    <tr key={user.id} className="hover:bg-[#EEF4ED]/30 transition-colors group">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          {user.avatarUrl ? (
                            <img src={user.avatarUrl} alt="" className="w-8 h-8 rounded-full object-cover" />
                          ) : (
                            <div className="w-8 h-8 rounded-full bg-[#EEF4ED] flex items-center justify-center text-[#134074] font-medium text-sm">
                              {user.firstName?.charAt(0) || '?'}
                            </div>
                          )}
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-[#134074]">{user.firstName} {user.lastName}</p>
                            <p className="text-xs text-[#134074] truncate">{user.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3">
                      ...
                      <td className="px-5 py-3 text-sm text-[#134074]">{new Date(user.createdAt).toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 transform transition-all animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-5 border-b border-[#C5D8E8]/10 flex justify-between items-center bg-[#EEF4ED]/30/50">
              <h3 className="text-lg font-semibold text-[#134074]">{t('admin.users.editTitle')}</h3>
              <button onClick={() => setEditUser(null)} className="p-2 hover:bg-[#EEF4ED] rounded-full transition-colors -mr-2">
                <span className="material-symbols-outlined text-[#13315C] text-[20px]">close</span>
              </button>
            </div>

            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4 p-4 bg-[#EEF4ED]/30 border border-[#C5D8E8]/10 rounded-xl">
                <div className="w-12 h-12 rounded-full bg-white ring-1 ring-[#C5D8E8]/20 flex items-center justify-center text-[#134074] font-bold text-lg shadow-sm">
                  {editUser.firstName?.charAt(0)}
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#134074]">{editUser.firstName} {editUser.lastName}</p>
                  <p className="text-xs text-[#134074] mt-0.5 font-medium">{editUser.email}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.users.firstName')}</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.users.lastName')}</label>
              ...
              <div>
                <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.users.email')}</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.users.phone')}</label>
                  <input type="tel" value={editForm.phone} onChange={e => setEditForm({ ...editForm, phone: e.target.value })} placeholder="+51 999 999 999"
                    className="w-full px-4 py-2.5 bg-white border border-[#C5D8E8]/20 rounded-xl text-sm focus:ring-2 focus:ring-[#134074]/10 focus:border-[#8DA9C4] outline-none transition-all leading-normal" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-[#134074] uppercase tracking-widest mb-1.5">{t('admin.users.role')}</label>
                  <div className="relative border border-[#C5D8E8]/20 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-[#134074]/10 focus-within:border-[#8DA9C4] transition-all">
                    <select value={editForm.role} onChange={e => setEditForm({ ...editForm, role: e.target.value })}
                      className="w-full px-4 py-2.5 bg-white text-sm appearance-none outline-none text-[#134074] cursor-pointer">
                    ...
                    <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-[#13315C] pointer-events-none text-[18px]">expand_content</span>
                  </div>
                </div>
              </div>
              ...
              <label className="flex items-center gap-3 p-3 pt-2 mt-2 cursor-pointer hover:bg-emerald-50/50 rounded-xl transition-colors border border-transparent hover:border-emerald-100 group">
                <input type="checkbox" checked={editForm.isVerified} onChange={e => setEditForm({ ...editForm, isVerified: e.target.checked })}
                  className="w-5 h-5 rounded border-[#C5D8E8]/30 text-emerald-500 focus:ring-emerald-500/20 transition-all cursor-pointer" />
                <div>
                  <span className="text-sm font-semibold text-[#134074] group-hover:text-emerald-800 transition-colors">{t('admin.users.isVerified')}</span>
                  <span className="text-[11px] text-[#134074] block leading-tight font-medium">{t('admin.users.isVerifiedDesc')}</span>
                </div>
              </label>
            </div>

            <div className="px-6 py-4 border-t border-[#C5D8E8]/10 flex justify-end gap-3 bg-[#EEF4ED]/30/50">
              <button onClick={() => setEditUser(null)} className="px-4 py-2 bg-white border border-[#C5D8E8]/20 text-sm font-medium text-[#134074] hover:bg-[#EEF4ED]/30 hover:text-[#134074] rounded-xl transition-colors shadow-sm">
                {t('common.cancel')}
              </button>
              <button onClick={handleSaveUser} disabled={saving}
                className="px-6 py-2 bg-[#134074] text-[#EEF4ED] rounded-xl text-sm font-semibold hover:bg-[#13315C] hover:-translate-y-0.5 shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none flex items-center gap-2">
