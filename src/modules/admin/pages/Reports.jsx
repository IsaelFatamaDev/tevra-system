...
export default function AdminReports() {
  ...
  const cityColors = ['bg-[#134074]', 'bg-[#8DA9C4]', 'bg-[#C5D8E8]', 'bg-[#C5D8E8]', 'bg-[#C5D8E8]/40']
    ...
    <div className="max-w-7xl mx-auto w-full space-y-5 platform-enter">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#134074] mt-0.5">{t('admin.reports.subtitle')}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="bg-[#EEF4ED] p-0.5 rounded-lg hidden sm:flex items-center">
            {[{ label: t('admin.reports.period7d'), value: '7d' }, { label: t('admin.reports.period30d'), value: '30d' }, { label: t('admin.reports.period1y'), value: '1y' }].map(f => (
              <button key={f.value} onClick={() => setPeriod(f.value)}
                className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${period === f.value ? 'bg-white shadow-sm text-[#134074]' : 'text-[#134074] hover:bg-white/60'}`}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        {metricCards.map((card, i) => (
          <div key={i} className="bg-white p-4 rounded-xl border border-[#C5D8E8]/20 stat-card">
            <div className="flex justify-between items-start mb-3">
              <div className="w-9 h-9 rounded-lg flex items-center justify-center bg-[#EEF4ED] text-[#134074]">
                <span className="material-symbols-outlined text-[18px]">{card.icon}</span>
              </div>
            </div>
            <p className="text-xs text-[#134074] mb-0.5">{card.title}</p>
            <h3 className="text-xl font-semibold text-[#134074]">{card.value}</h3>
            <p className="text-[10px] text-[#13315C] mt-1">{card.subtitle}</p>
          </div>
        ))}
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-white p-5 rounded-xl border border-[#C5D8E8]/20 relative overflow-hidden">
          <div className="flex justify-between items-start mb-5 relative z-10">
            <div>
              <h4 className="text-sm font-semibold text-[#134074]">{t('admin.reports.salesGrowth')}</h4>
              <p className="text-xs text-[#134074]">{t('admin.reports.monthlyTrend')}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-[#134074]"></span>
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group">
                    <div className="relative w-full flex justify-center">
                      <div className="absolute -top-6 opacity-0 group-hover:opacity-100 transition-opacity bg-[#134074] text-[#EEF4ED] text-[10px] px-2 py-0.5 rounded font-medium whitespace-nowrap">
                        ${Number(m.revenue).toLocaleString()}
                      </div>
                    </div>
                    <div className="w-full max-w-8 rounded-t-md bg-[#C5D8E8]/40 hover:bg-[#134074] transition-colors"
                      style={{ height: `${Math.max(pct, 3)}%` }}></div>
                    <span className="text-[9px] sm:text-[10px] text-[#134074] font-medium mt-1 capitalize">{monthLabel}</span>
                  </div>
                )
              })
            })() : (
              <div className="flex items-center justify-center w-full h-full text-[#13315C] text-sm">{t('admin.reports.noSalesData')}</div>
            )}
          </div>
        </div>

        <div className="bg-white p-5 rounded-xl border border-[#C5D8E8]/20 flex flex-col">
          <h4 className="text-sm font-semibold text-[#134074] mb-1">{t('admin.reports.byCategory')}</h4>
          <p className="text-xs text-[#134074] mb-5">{t('admin.reports.sectorDistribution')}</p>
          <div className="flex-1 flex flex-col items-center justify-center relative">
            {(() => {
              const catColors = ['bg-[#C5D8E8]/20', 'bg-[#C5D8E8]', 'bg-[#8DA9C4]', 'bg-[#13315C]', 'bg-[#134074]', 'bg-[#C5D8E8]']
              ...
              return (
                <>
                  <div className="w-40 h-40 sm:w-48 sm:h-48 rounded-full bg-[#EEF4ED]/30 relative flex items-center justify-center overflow-hidden">
                    {catData.length > 0 ? (
                    ...
                    <div className="text-center z-10">
                      <span className="text-xl sm:text-2xl font-semibold text-[#134074]">{categories.length}</span>
                      <p className="text-[10px] text-[#134074] font-medium uppercase">{t('admin.reports.categoriesLabel')}</p>
                    </div>
                  </div>
                  <div className="mt-5 grid grid-cols-2 gap-x-4 gap-y-2 w-full">
                    {catData.map((c, i) => (
                      <div key={i} className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs text-[#134074] truncate">{c.name}</span>
                        <span className="text-xs font-medium text-[#134074] ml-auto">{c.pct}%</span>
                      </div>
                    ))}
                    {catData.length === 0 && <p className="col-span-2 text-xs text-[#13315C] text-center">{t('admin.reports.noData')}</p>}
                  </div>
                </>
              )
            })()}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-5">
        <div className="lg:col-span-3 bg-white rounded-xl border border-[#C5D8E8]/20 overflow-hidden">
          <div className="p-4 border-b border-[#C5D8E8]/10 flex justify-between items-center">
            <h4 className="text-sm font-semibold text-[#134074]">{t('admin.reports.topGrowthAgents')}</h4>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[500px]">
              <thead>
                <tr className="bg-[#EEF4ED]/30 text-[11px] font-medium uppercase tracking-wider text-[#134074] border-b border-[#C5D8E8]/10">
              ...
              <tbody className="divide-y divide-[#C5D8E8]/10">
                {topAgents.map((agent, i) => (
                  <tr key={i} className="hover:bg-[#EEF4ED]/30 transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-7 h-7 rounded-full bg-[#EEF4ED] flex items-center justify-center font-medium text-xs text-[#134074]">{i + 1}</div>
                        <span className="text-sm font-medium text-[#134074]">{agent.displayName}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3 text-sm font-medium text-[#134074]">{Number(agent.totalOrders).toLocaleString()}</td>
        ...
        <div className="lg:col-span-2 bg-white p-5 rounded-xl border border-[#C5D8E8]/20">
          <h4 className="text-sm font-semibold text-[#134074] mb-5">{t('admin.reports.topCities')}</h4>
          <div className="space-y-4">
            {cities.slice(0, 5).map((city, i) => (
              <div key={i} className="space-y-1.5">
                <div className="flex justify-between text-sm">
                  <span className="font-medium text-[#134074]">{city.city || t('admin.reports.noCity')}</span>
                  <span className="text-[#134074]">{Number(city.totalOrders).toLocaleString()} {t('admin.reports.ordersCount')}</span>
                </div>
                <div className="w-full h-1.5 bg-[#EEF4ED] rounded-full overflow-hidden">
