    ...
    <div className="space-y-6 platform-enter">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant/10">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-text-muted capitalize">{user?.role === 'customer' ? 'Cliente' : user?.role}</p>
            </div>
          </div>

          {profileMsg && (
            <div className={`mb-4 p-3 rounded-xl text-sm font-medium ${profileMsg.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
              {profileMsg.text}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-outline-variant/20 bg-surface-container-low/50 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
      ...
      {tab === 'password' && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant/10 max-w-lg">
          ...
          {pwMsg && (
            <div className={`mb-4 p-3 rounded-xl text-sm font-medium ${pwMsg.type === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
              {pwMsg.text}
            </div>
          )}
